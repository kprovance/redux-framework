...
(function($) {
	...
	var decoration      = $('#' + mainID + ' select.redux-typography-decoration').val(); // New Text Decoration
        ...
        if (isSelecting == true) {
            var google = $.reduxTypography.makeBool(selVals.object['data-google']);
            $('#' + mainID + ' .redux-typography-google-font').val(google);
        } else {
            var google = $.reduxTypography.makeBool($('#' + mainID + ' .redux-typography-google-font').val()); // Check if font is a google font
        }
